...
#d:: 
    {
        ...
        if (ActiveHwnd) {
            ...
            else if(activeClass = "OpusApp" && activeProcess = "WINWORD.EXE") {
                Last_winword := ActiveHwnd
                Last_Hwnd := ActiveHwnd
            } else if (activeClass = "MozillaWindowClass" && activeProcess = "firefox.exe") {
                Last_firefox := ActiveHwnd
                Last_Hwnd := ActiveHwnd
            } else if (activeClass = "CASCADIA_HOSTING_WINDOW_CLASS" && activeProcess = "WindowsTerminal.exe") {
                Last_terminal := ActiveHwnd
                Last_Hwnd := ActiveHwnd
            } else if (activeClass = "Chrome_WidgetWin_1" && activeProcess = "哔哩哔哩.exe") {
                Last_bilibili := ActiveHwnd
                Last_Hwnd := ActiveHwnd
            } else if (activeClass = "Chrome_WidgetWin_1" && activeProcess = "Code.exe") {
                Last_code := ActiveHwnd
                Last_Hwnd := ActiveHwnd
            } else if (activeClass = "READER" && activeProcess = "Reader.exe") {
                Last_reader := ActiveHwnd
                Last_Hwnd := ActiveHwnd
            }
